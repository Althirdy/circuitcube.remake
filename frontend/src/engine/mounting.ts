...
import { icMountCandidate, icMountPosition } from './icMounting';
import { isLogicIc } from './digital/logicIcDefinitions';

export const mountedBoardId = (instance: ComponentInstance) => instance.icMount?.breadboardId ?? instance.mount?.breadboardId ?? instance.switchMount?.breadboardId ?? instance.resistorMount?.breadboardId;
export function mountedPosition(instance: ComponentInstance, instances: ComponentInstance[], sockets: SocketSource) {
  if (instance.icMount) return icMountPosition(instance.icMount, instances, sockets);
...
export function mountFlipped(instance: ComponentInstance | undefined, layout: Layout, source: SocketSource) {
  ...
  if (instance.icMount) return instance.icMount.pins[0].startsWith('f');
...
export function mountingCandidate(layout: Layout, modelId: ModelId, board: ComponentInstance, point: Point3, source: SocketSource, flipped: boolean, ignoreId?: string): MountCandidate {
  if (isLogicIc(modelId)) return icMountCandidate(layout, board, point, source, flipped, ignoreId);
  ...
  if (!nearest?.row) return { kind: 'invalid', mount: null, valid: false, reason: 'Use terminal holes, not rails' };
  if (Math.hypot(nearest.position[0] - point[0], nearest.position[2] - point[2]) > SOCKET_PITCH * 0.72) return { kind: 'invalid', mount: null, valid: false, reason: 'Move onto a terminal strip; the trench is not a socket' };
  ...
  if (modelId === 'resistor') {
    ...
    return { kind: 'resistor', mount: null, resistorMount, valid: !error, reason: error ?? `Resistor: ${pins[0]} ↔ ${pins[1]} · non-polarized` };
  }
  if (modelId === 'slide-switch') {
    ...
    return { kind: 'switch', mount: null, switchMount, valid: !error, reason: error ?? `1: ${pins[0]} / 2 common: ${pins[1]} / 3: ${pins[2]}` };
  }
  ...
  return { kind: 'led', mount, valid: !error, reason: error ?? `Anode + ${mount.anode} / Cathode − ${mount.cathode}` };
}
