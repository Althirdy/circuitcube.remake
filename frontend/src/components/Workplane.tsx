...
import { mountedPosition } from '../engine/mounting';
    ...
    >
      ...
      {selected && !workspace.placing && (
        <div className={`selection-panel${isLogicIc(selected.modelId) ? ' ic-selection-panel' : ''}`}>
          ...
          {selected.resistorMount && <button className="detach-resistor" onClick={() => workspace.detach(selected.id)}>Detach resistor</button>}
